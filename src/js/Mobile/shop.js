...
function shop_item_init(cart_only){
    ...
    for(var i=0; i<cache_keys.length; i++){
        ...
        if(parseInt(item_list['count']) < parseInt(item_list['min'])){
            ...
            $('.tooltiptext').text(item_list['min']+'杯起送，挑多几杯吧');
            ...
            if(tmp_num >= 0 && tmp_num%gcd != 0){
                ...
                $('.cart-btn').addClass('weui-btn_disabled');
                $('.tooltiptext').css("visibility", "visible");
                $('.tooltiptext').text('满8/12/16杯才能下单哦~');
            }
        }
...
$(function() {
    ...
    $('body').on('click', '.item-minus', function(e){
        ...
        if(!isEmpty(sel_cache[column_guid]) &&sel_cache[column_guid]['count'] < parseInt(sel_cache[column_guid]['min']) && sel_cache[column_guid]['count']>0){
            ...
            $('.cart-btn').addClass('weui-btn_disabled');
            $('.tooltiptext').css("visibility", "visible");
            $('.tooltiptext').text(sel_cache[column_guid]['min']+'杯起送，挑多几杯吧');
    ...
    $('.cart-btn').on('click', function(e){
        ...
        if($(this).attr('params') == 'send_yourself'){
            $.post("/Shop/Index/add_to_card_handle", sel_cache, function(data) {
                if (data['status'] == 0) {
                    window.location.href = "/Shop/Index/cart";
                    ...
                    if ($('#Binding-mobile-phone').css('display') == 'none'){
                        ...
                        $("#getVerificationCode").addClass("weui-btn_disabled weui-btn_default");
        ...
        if($(this).attr('params') == 'send_friend'){
            $.post("/Shop/Index/add_to_card_handle", sel_cache, function(data){
                if(data['status'] == 0){
                    window.location.href = "/Shop/Index/send_friend";
                }else if(data['status'] == -30004){
                    if($('#Binding-mobile-phone').css(display) == 'none'){
                        $('#Binding-mobile-phone').show(100);
                        $(".bind_mobile-btn").addClass("weui-btn_disabled weui-btn_default");
                ...
                <div>\
                    <div class="placeholder">\
                        <span class="item-minus item-change">\
                            <button class="weui-btn weui-btn_mini item-minus" column="'+column_guid+'" item="'+guid+'"><i class="iconfont icon-jianhao"></i></button>\
                        </span>\
                        <span class="goods-number number-'+guid+'">'+num+'</span>\
                        <span class="item-plus item-change">\
                            <button class="weui-btn weui-btn_mini item-plus" column="'+column_guid+'" item="'+guid+'"><i class="iconfont icon-jiahao1"></i></button>\
                        </span>\
                    </div>\
                </div>\
            </div>';
...
function goTop(acceleration, time) {
    acceleration = acceleration || 0.1;  //加速度设置
    time = time || 6;   //滑动时间，调多了会出现滑动bug
    var x1 = 0;
    var y1 = 0;
    var x2 = 0;
    var y2 = 0;
...
var countdown = 60;  //设定120秒刷新
function settime(val) {
    if (countdown == 0) {
        val.removeAttr("disabled"); //移除disabled属性
        val.text("重新发送");
        val.css({"color":"#d4a77E"});
        val.removeClass("weui-btn_disabled weui-btn_default");
        countdown = 60;
        return false;
    } else {
...
$("#getVerificationCode").on('click', function(){
    ...
    document.getElementById("getVerificationCode").addEventListener("click",isPhoneNum);
    //判断验证码按钮是否可以点击
    if($(".phonenum").val() == ""){ //手机号为空
        $("#getVerificationCode").attr("disabled", true); //获取验证码按钮不可点击
        return false;
    }

    $('.verificationCode').removeAttr("disabled");
    settime($("#getVerificationCode"));

    $.post("/Shop/Index/get_to_code",mobile_code, function(data){ //另起一个post,只传手机号以及验证码
        if(data['status'] != 0){
            $('.error-message').text(data['message']);
            $('.error-dialog').show();
...
function isPhoneNum(){
    ...
    if(!myreg.test(phonenum)){
        ...
        getVerificationCode.removeClass("weui-btn_disabled weui-btn_default");
        return true;
    }
}
//取消绑定手机号
...
$('.bind_mobile-btn').on('click', function(){
    ...
    $.post("/Shop/Index/add_to_tel",mobile_code, function(data){ //另起一个post,只传手机号以及验证码
        if(data['status'] != 0){
            $('.error-message').text(data['message']);
            $('.error-dialog').show();
    ...
    document.getElementById("getVerificationCode").removeEventListener("click",isPhoneNum);
});

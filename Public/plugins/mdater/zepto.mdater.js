(function($){
	$.fn.mdater = function(config){
		...
		var mdater = {
			...
			_finish_function : option.finish_function,
			//保存上一次选择的数据
			...
			refreshView : function(){
				var initVal = option.correct_date,
					date = null;
				if(initVal){
			...
			afterShowPanel : function (mask,panel) {
				...
				panel.delegates({
					...
					'.md_ok' : function(){
						...
						_this._finish_function(_this.value.year + '-' + monthValue + '-' + dateValue);
						_this._hidePanel();
					},
					'.md_cancel' : function(){
						_this._hidePanel();
